    ...
    if error_msg:
        ...
        if isinstance(hta_data, dict) and "hta_tree" in hta_data:
            ...
            if isinstance(hta_tree_content, dict):
                ...
                logger.warning(
                    f"HTA 'hta_tree' has unexpected type: {type(hta_tree_content)}"
                )
                st.session_state[KEY_HTA_STATE] = None
        ...
        logger.error(
            f"Failed fetch HTA: Status {status_code}. Data: {str(hta_data)[:200]}"
        )
        st.session_state[KEY_ERROR_MESSAGE] = (
            f"Unexpected API status ({status_code}) fetching HTA."
        )
        st.session_state[KEY_HTA_STATE] = None
...
STATUS_COLORS = {
    ...
    STATUS_ACTIVE: "#ADD8E6",
    STATUS_COMPLETED: "#90EE90",
    STATUS_PRUNED: "#A9A9A9",
    STATUS_BLOCKED: "#FFCCCB",
    "default": "#FFFFFF",
}
...
def build_hta_dot_string(node_data: Dict[str, Any], dot: graphviz.Digraph):
    ...
    if not node_id:
        logger.warning("Skip HTA node missing 'id'.")
        return
    node_title = node_data.get("title", "Untitled")
    node_status = str(node_data.get("status", "default")).lower()
    ...
    if isinstance(children, list):
        for child_data in children:
            if isinstance(child_data, dict):
                ...
                logger.warning(f"Skip non-dict child under node {node_id}.")
...
def display_hta_visualization(hta_tree_root: Optional[Dict]):
    if not hta_tree_root or not isinstance(hta_tree_root, dict):
        ...
        return
    try:
        dot = graphviz.Digraph(comment="HTA Skill Tree")
        dot.attr(rankdir="TB")
        build_hta_dot_string(hta_tree_root, dot)
        st.graphviz_chart(dot)
        st.caption("Current Skill Tree Visualization")
    except Exception as e:
        logger.exception("HTA viz render exception!")
        st.error(f"Error generating HTA viz: {e}")
...
def handle_completion_confirmation():
    ...
    if not node_id_to_confirm:
        logger.error("Confirm missing 'hta_node_id'.")
    ...
    with col_confirm:
        if st.button("✅ Yes", key=f"confirm_yes_{node_id_to_confirm}"):
            st.session_state[KEY_ERROR_MESSAGE] = None
            logger.info(f"User confirm node: {node_id_to_confirm}")
            ...
            if response.get(KEY_ERROR):
                error_msg = response.get(KEY_ERROR, "Unknown")
                logger.error(
                    f"API error confirm task {node_id_to_confirm}: {error_msg}"
                )
                st.error(f"Confirm Fail: {error_msg}")
                st.session_state[KEY_ERROR_MESSAGE] = f"API Error: {error_msg}"
            elif response.get(KEY_STATUS_CODE) == 200:
                st.success("Confirmed!")
                logger.info(f"Success confirm node {node_id_to_confirm}.")
                st.session_state[KEY_PENDING_CONFIRMATION] = None
                resp_data = response.get(KEY_DATA, {})
                if isinstance(resp_data, dict):
                    completion_message = resp_data.get(
                    ...
                    if completion_message:
                        ...
                        st.session_state.messages.append(
                            {"role": "assistant", "content": str(completion_message)}
                        )
                    challenge_data = resp_data.get("result", {}).get(
                        KEY_COMMAND_MASTERY
                    )
                    if isinstance(challenge_data, dict):
                        challenge_content = challenge_data.get(
                            "challenge_content", "Consider next steps."
                        )
                        challenge_type = challenge_data.get("challenge_type", "Reflect")
                        ...
                        st.session_state.messages.append(
                            {
                                "role": "assistant",
                                "content": f"✨ Mastery Challenge ({challenge_type}):\n{challenge_content}",
                            }
                        )
                fetch_hta_state()
                st.rerun()
                ...
                logger.error(
                    f"Unexpected status {response.get(KEY_STATUS_CODE)} confirm task {node_id_to_confirm}."
                )
                st.error(
                    f"Unexpected confirm status ({response.get(KEY_STATUS_CODE)})."
                )
                st.session_state[KEY_ERROR_MESSAGE] = (
                    f"API Error: Confirm status {response.get(KEY_STATUS_CODE)}"
                )
    with col_deny:
        if st.button("❌ No", key=f"confirm_no_{node_id_to_confirm}"):
            st.session_state[KEY_ERROR_MESSAGE] = None
            logger.info(f"User denied node: {node_id_to_confirm}")
            st.info("Okay, task not marked complete.")
            st.session_state[KEY_PENDING_CONFIRMATION] = None
            if not isinstance(st.session_state.get(KEY_MESSAGES), list):
                st.session_state[KEY_MESSAGES] = []
            st.session_state.messages.append(
                {"role": "assistant", "content": "Okay, let me know when you're ready."}
            )
            st.rerun()
...
st.set_page_config(
    page_title="Forest OS", layout="wide", initial_sidebar_state="expanded"
)
...
st.session_state.setdefault(KEY_HTA_STATE, None)
st.session_state.setdefault(KEY_PENDING_CONFIRMATION, None)
        ...
        with st.expander("Snapshot Management", expanded=False):
            ...
            if st.button("Refresh Snapshot List"):
                ...
                with st.spinner("Fetching snapshot list..."):
                    ...
                    if response.get(KEY_ERROR):
                        ...
                        st.session_state[KEY_ERROR_MESSAGE] = (
                            f"API Error: {response.get(KEY_ERROR)}"
                        )
                    elif response.get(KEY_STATUS_CODE) == 200 and isinstance(
                        response.get(KEY_DATA), list
                    ):
                        snapshot_list_data = response.get(KEY_DATA, [])
                        valid_snapshots = [
                            item
                            for item in snapshot_list_data
                            if isinstance(item, dict)
                            and KEY_SNAPSHOT_ID in item
                            and KEY_SNAPSHOT_UPDATED_AT in item
                        ]
                        try:
                            st.session_state.snapshots = sorted(
                                valid_snapshots,
                                key=lambda x: datetime.fromisoformat(
                                    str(x[KEY_SNAPSHOT_UPDATED_AT]).replace(
                                        "Z", "+00:00"
                                    )
                                ),
                                reverse=True,
                            )
                            if not st.session_state.snapshots and snapshot_list_data:
                                st.warning("Snapshot list format unexpected.")
                            elif not snapshot_list_data:
                                st.info("No snapshots found.")
                        except Exception as sort_e:
                            logger.error(f"Snapshot sort fail: {sort_e}")
                            st.session_state.snapshots = valid_snapshots
                            st.warning("Snapshots loaded, sort fail.")
                    else:
                        status = response.get(KEY_STATUS_CODE, "N/A")
                        data_type = type(response.get(KEY_DATA)).__name__
                        st.error(
                            f"Fetch Snap Fail: Status {status}, Format: {data_type}"
                        )
                        st.session_state[KEY_ERROR_MESSAGE] = (
                            f"Snapshot API Error: Status {status}, Format {data_type}"
                        )
            ...
            if st.session_state.get("snapshots"):
                ...
                for s in st.session_state.snapshots:
                    ...
                    if updated_at_raw:
                        try:
                            ...
                            logger.warning(f"Parse date fail: {updated_at_raw}")
                    display_key = (
                        f"'{codename}' ({dt_str}) - ID: ...{str(snap_id)[-6:]}"
                    )
                ...
                if snapshot_options:
                    ...
                    snapshot_id_selected = snapshot_options.get(
                        selected_snapshot_display
                    )
                    col_load, col_delete = st.columns(2)
                    with col_load:
                        if st.button(
                            "Load Selected", key="load_snap", help="Load session state"
                        ):
                            st.session_state[KEY_ERROR_MESSAGE] = None
                            if snapshot_id_selected:
                                with st.spinner(
                                    f"Loading '{selected_snapshot_display}'..."
                                ):
                                    ...
                                    if response.get(KEY_ERROR):
                                        st.error(
                                            f"Load Fail: {response.get(KEY_ERROR)}"
                                        )
                                        st.session_state[KEY_ERROR_MESSAGE] = (
                                            f"API Error: {response.get(KEY_ERROR)}"
                                        )
                                        ...
                                        response.get(KEY_DATA), dict
                                    ):
                                        load_data = response.get(KEY_DATA, {})
                                        st.success(load_data.get("message", "Loaded!"))
                                        logger.info(
                                            f"Snapshot {snapshot_id_selected} loaded."
                                        )
                                        st.session_state[KEY_MESSAGES] = (
                                            load_data.get(KEY_MESSAGES, [])
                                            if isinstance(
                                                load_data.get(KEY_MESSAGES), list
                                            )
                                            else []
                                        )
                                        st.session_state[KEY_CURRENT_TASK] = (
                                            load_data.get(KEY_CURRENT_TASK)
                                            if isinstance(
                                                load_data.get(KEY_CURRENT_TASK), dict
                                            )
                                            else None
                                        )
                                        st.session_state[KEY_MILESTONES] = (
                                            load_data.get(KEY_MILESTONES, [])
                                            if isinstance(
                                                load_data.get(KEY_MILESTONES), list
                                            )
                                            else []
                                        )
                                        st.session_state[KEY_ERROR_MESSAGE] = None
                                        ...
                                        st.session_state[KEY_HTA_STATE] = None
                                        st.session_state[KEY_PENDING_CONFIRMATION] = (
                                            None
                                        )
                                        fetch_hta_state()
                                        st.rerun()
                                        ...
                                        status = response.get(KEY_STATUS_CODE, "N/A")
                                        st.error(f"Load Fail: Status {status}")
                                        st.session_state[KEY_ERROR_MESSAGE] = (
                                            f"Snapshot Load API Error: Status {status}"
                                        )
                            else:
                    ...
                    with col_delete:
                        if st.button(
                            ...
                            if snapshot_id_selected:
                                with st.spinner(
                                    f"Deleting '{selected_snapshot_display}'..."
                                ):
                                    delete_endpoint = (
                                        f"/snapshots/{snapshot_id_selected}"
                                    )
                                    response = call_forest_api(
                                        delete_endpoint,
                                        method="DELETE",
                                    ...
                                    if response.get(KEY_ERROR):
                                        st.error(
                                            f"Delete Fail: {response.get(KEY_ERROR)}"
                                        )
                                        st.session_state[KEY_ERROR_MESSAGE] = (
                                            f"API Error: {response.get(KEY_ERROR)}"
                                        )
                                        ...
                                        st.success("Deleted.")
                                        logger.info(
                                            f"Snapshot {snapshot_id_selected} deleted."
                                        )
                                        if isinstance(
                                            st.session_state.get("snapshots"), list
                                        ):
                                            st.session_state.snapshots = [
                                                s
                                                for s in st.session_state.snapshots
                                                if s.get(KEY_SNAPSHOT_ID)
                                                != snapshot_id_selected
                                            ]
                                        st.rerun()
                                    else:
                                        status = response.get(KEY_STATUS_CODE, "N/A")
                                        st.error(f"Delete Fail: Status {status}")
                                        st.session_state[KEY_ERROR_MESSAGE] = (
                                            f"Snapshot Delete API Error: Status {status}"
                                        )
                            else:
                                st.warning("No snapshot selected.")
                else:
                    if (
                        "snapshots" in st.session_state
                        and not st.session_state.snapshots
                    ):
                        pass
                    else:
                        st.caption("Click 'Refresh' to load snapshots.")
...
if auth_action_taken:
    if (
        ...
        and st.session_state.get(KEY_ONBOARDING_STATUS)
        == constants.ONBOARDING_STATUS_COMPLETED
        and not st.session_state.get(KEY_HTA_STATE)
    ):
...
if not st.session_state.get("authenticated"):
    ...
    st.image(
        "https://placehold.co/800x400/334455/FFFFFF?text=Welcome+to+Forest+OS",
        caption="Visualize your growth journey",
    )
else:
    # Check/Fetch User Status if missing
    if st.session_state.get(KEY_ONBOARDING_STATUS) is None and st.session_state.get(
        "token"
    ):
        ...
        with st.spinner("Checking status..."):
            ...
            if user_details_response.get(KEY_ERROR) or not isinstance(
                ...
                st.error("Status retrieval failed. Try logout/login.")
                ...
                if user_onboarding_status in valid_statuses:
                    ...
                    logger.info(
                        f"Refreshed status: {st.session_state[KEY_ONBOARDING_STATUS]}"
                    )
                    if st.session_state[
                        KEY_ONBOARDING_STATUS
                    ] == constants.ONBOARDING_STATUS_COMPLETED and not st.session_state.get(
                        KEY_HTA_STATE
                    ):
                        fetch_hta_state()
                    st.rerun()
                else:
                    logger.error(f"Invalid status received: {user_onboarding_status}")
                    st.error("Invalid status from backend.")
                    st.session_state[KEY_ONBOARDING_STATUS] = "error"
    ...
    with col_hta:
        ...
        if (
            ...
            st.info("Viz disabled.")
        elif onboarding_status in [
            constants.ONBOARDING_STATUS_NEEDS_GOAL,
            constants.ONBOARDING_STATUS_NEEDS_CONTEXT,
        ]:
            st.info("Complete onboarding to view Skill Tree.")
        elif onboarding_status == "error":
            st.warning("Cannot display Skill Tree due to status error.")
    ...
    with col_chat:
        ...
        if isinstance(messages_list, list):
            for message in messages_list:
                if (
                    ...
                    logger.warning(f"Skip invalid message: {message}")
        elif messages_list:
            logger.error(f"Chat history not list: {type(messages_list)}")
            st.error("Chat history corrupt.")
        ...
        # --- Main Interaction Input ---
            ...
            if prompt := st.chat_input(
                input_placeholder, disabled=chat_disabled, key="main_chat"
            ):
                st.session_state[KEY_ERROR_MESSAGE] = None
                if not isinstance(st.session_state.get(KEY_MESSAGES), list):
                    st.session_state[KEY_MESSAGES] = []
                ...
                with st.chat_message("assistant"):
                    ...
                    if response.get(KEY_ERROR):
                        ...
                        logger.warning(
                            f"Error {api_endpoint}: {error_msg} (Status: {response.get(KEY_STATUS_CODE)})"
                        )
                    ...
                    elif response.get(KEY_STATUS_CODE) in [200, 201] and isinstance(
                        response.get(KEY_DATA), dict
                    ):
                        resp_data = response.get(KEY_DATA, {})
                        logger.info(f"Success from {api_endpoint}.")
                        assistant_response_content = resp_data.get(
                            KEY_COMMAND_RESPONSE, resp_data.get("message", "")
                        )
                        new_task_data = resp_data.get(KEY_CURRENT_TASK)
                        st.session_state[KEY_CURRENT_TASK] = (
                            new_task_data if isinstance(new_task_data, dict) else None
                        )
                        ...
                        if not isinstance(st.session_state.get(KEY_MILESTONES), list):
                            st.session_state[KEY_MILESTONES] = []
                        st.session_state.milestones_achieved.append(
                            str(milestone_feedback)
                        )
                        assistant_response_content += (
                            f"\n\n🎉 *Milestone: {milestone_feedback}*"
                        )
                        if action_required == "confirm_completion" and isinstance(
                            confirmation_details, dict
                        ):
                            logger.info(
                                f"Confirm requested: {confirmation_details.get('hta_node_id')}"
                            )
                            st.session_state[KEY_PENDING_CONFIRMATION] = (
                                confirmation_details
                            )
                            if not assistant_response_content:
                                assistant_response_content = confirmation_details.get(
                                    "prompt", "Confirm?"
                                )
                        else:
                        ...
                        data_type = type(response.get(KEY_DATA)).__name__
                        logger.error(
                            f"Unexpected response {api_endpoint}: Status {status_code}, Type: {data_type}"
                        )
                        assistant_response_content = (
                            f"Unexpected server response (Status: {status_code})."
                        )
                        st.session_state[KEY_ERROR_MESSAGE] = assistant_response_content
        ...
        # --- Handle Other/Error States ---
            ...
            st.info("Checking status...")
        elif current_status == "error":
            st.error("Status error. Try logout/login.")
        else:
            st.warning(f"Unknown state ('{current_status}'). Try logout/login.")
