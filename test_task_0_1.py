...
class Task01TestSuite(unittest.TestCase):
    ...
    def test_secret_key_configured(self):
        ...
        self.assertTrue(
            len(secret_key) >= 16, "SECRET_KEY should be at least 16 characters"
        )
    ...
    def test_database_connection_string(self):
        ...
        logger.info("✅ Database connection string found: %s...", db_connection[:20])
    ...
    def test_feature_flags(self):
        ...
        self.assertEqual(
            core_onboarding, "true", "FEATURE_ENABLE_CORE_ONBOARDING should be 'true'"
        )
        self.assertEqual(core_hta, "true", "FEATURE_ENABLE_CORE_HTA should be 'true'")
        self.assertEqual(
            core_task_engine, "true", "FEATURE_ENABLE_CORE_TASK_ENGINE should be 'true'"
        )

        logger.info(
            f"✅ Feature flags correctly configured: "
    ...
    def test_runtime_txt(self):
        ...
        self.assertTrue(
            runtime_content.startswith("python-3.11"),
            f"runtime.txt should specify Python 3.11.x, found: {runtime_content}",
        )
        logger.info(
            f"✅ runtime.txt contains correct Python version: {runtime_content}"
        )
    ...
    def test_documentation_files(self):
        ...
        for doc_file in doc_files:
            ...
            self.assertTrue(
                file_path.exists(), f"Documentation file not found: {doc_file}"
            )

            # Check file has content
            self.assertTrue(
                file_path.stat().st_size > 0, f"Documentation file is empty: {doc_file}"
            )
        ...
        try:
            ...
            with engine.connect() as connection:
                ...
                for table in core_tables:
                    ...
                    self.assertTrue(
                        result.scalar(), f"{table} table not found in database"
                    )
    ...
    def test_settings_file(self):
        ...
        self.assertTrue(
            "os.getenv" in settings_content and "SECRET_KEY" in settings_content,
