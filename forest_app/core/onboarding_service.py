...
class OnboardingService:
    ...
    class DynamicQuestion(BaseModel):
        ...
        required: bool = Field(
            default=False, description="Whether this question must be answered"
        )
    ...
    async def get_dynamic_questions(
        self, goal: str, context: str = None
    ...
    def process_qa_responses(
        self, goal: str, context: str, qa_responses: Dict[str, str]
    ) -> Dict[str, Any]:
        """
        ...
        result = {
            ...
            "q_and_a_responses": [
                {
                    "question_key": key,
                    "question": self._predefined_questions.get(
                        key, OnboardingQuestion(text="Unknown", key=key)
                    ).text,
                    "response": answer,
                }
