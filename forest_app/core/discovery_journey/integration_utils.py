...
async def enrich_hta_with_discovery_insights(
    ...
    try:
        ...
        abstraction_level = await discovery_service.assess_abstraction_level(
            user_id=user_id, goal_description=raw_goal, context_reflection=raw_context
        )

        # If the goal is abstract, subtly modify the HTA to include exploratory elements
        if (
            abstraction_level and abstraction_level.get("level", 0) > 6
        ):  # On a scale of 1-10
            ...
            if root_id:
                ...
                if trunk_nodes:
                    target_node = _select_exploration_target(
                        trunk_nodes, abstraction_level
                    )

                    # Generate exploratory tasks
                    exploratory_tasks = (
                        await discovery_service.generate_exploratory_tasks(
                    ...
                    for task in exploratory_tasks:
                        ...
                        task["metadata"]["abstraction_level"] = abstraction_level.get(
                            "level"
                        )
                        ...
                        tree_obj.add_node(
                            node_id=task.get("id")
                            or f"discovery_{task.get('title', '')[:10]}_{id(task)}",
        ...
        logger.warning(
            "Non-critical: Could not enrich HTA with discovery insights: %s", e
        )
        return hta_tree


def _select_exploration_target(
    nodes: List[Any], abstraction_data: Dict[str, Any]
) -> Any:
    """
    ...
    if abstract_areas:
        for area in abstract_areas:
            for node in nodes:
                ...
                if (
                    area.lower() in node.title.lower()
                    or area.lower() in node.description.lower()
                ):
                    return node
...
async def track_task_completion_for_discovery(
    ...
    try:
        ...
        await discovery_service.process_task_completion(
            user_id=user_id, task_id=task_id, feedback=feedback or {}
        )
    except Exception as e:
        logger.warning(
            "Non-critical: Could not track task completion for discovery: %s", e
        )
...
async def infuse_recommendations_into_snapshot(
    ...
    try:
        ...
        evolution = await discovery_service.evolve_focus_based_on_patterns(
            user_id=user_id, tree=tree
        )

        if evolution:
            # Store evolution data invisibly in the snapshot
            if "discovery_journey" not in snapshot.component_state:
            ...
            if recommended_tasks and tree.root_id:
                ...
                # Limit to 2 to avoid overwhelming
                for task in recommended_tasks[:2]:
        ...
        logger.warning(
            "Non-critical: Could not infuse recommendations into snapshot: %s", e
        )
        return snapshot
...
def _find_appropriate_target_node(tree: HTATree, task: Dict[str, Any]) -> Optional[str]:
    ...
    for node in tree.nodes.values():
        ...
        if task_theme in node.title.lower() or task_theme in node.description.lower():
            return node.node_id

    # If no good match, return a trunk node
    trunk_nodes = [
        node.node_id for node in tree.nodes.values() if node.parent_id == root_id
    ]
