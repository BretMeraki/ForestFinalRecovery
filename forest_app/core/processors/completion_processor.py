...
class CompletionProcessor:
    ...
    async def process_node_completion(
        ...
        try:
            ...
            if node.user_id != user_id:
                self.logger.error("User %s does not own node %s", user_id, node_id)
                raise ValueError("User %s does not own node %s", user_id, node_id)

            # 2. Check if already completed (idempotency)
            if node.status == "completed":
                self.logger.info("Node %s already completed", node_id)
                # Find existing reinforcement message if available
                existing_footprint = await self.tree_repository.get_task_footprint(
                    node_id
                )
                message = (
                    existing_footprint.reinforcement_message
                    if existing_footprint
                    else "This task is already completed."
                )
                return {
            ...
            if not tree:
                self.logger.error("Tree %s not found", node.tree_id)
                raise ValueError("Tree %s not found", node.tree_id)
            ...
            if not success:
                self.logger.error("Failed to update node %s status", node_id)
                raise RuntimeError("Failed to update node %s status", node_id)
            ...
            if node.roadmap_step_id:
                ...
                manifest = manifest.update_step_status(
                    node.roadmap_step_id, "completed"
                )
            ...
            if node.parent_id:
                ...
                if increment_success:
                    self.logger.info(
                        "Incremented completion count for parent %s to %s",
                        node.parent_id,
                        new_count,
                    )
            ...
            await self.memory_manager.update_memory_with_completion(
            ...
            result = {
                ...
                "roadmap_step_id": (
                    str(node.roadmap_step_id) if node.roadmap_step_id else None
                ),
            }

            self.logger.info(
                "Successfully completed node %s for user %s", node_id, user_id
            )
            return result

        except Exception as e:
            self.logger.error("Error processing node completion: %s", e)
        ...
        try:
            ...
            if node.is_major_phase:
                ...
                templates = [
                    ...
                    "Task complete! You're making steady progress.",
            ...
            self.logger.warning("Error generating reinforcement message: %s", e)
    ...
    async def process_completion(
        ...
        try:
            ...
            if self.semantic_memory_manager:
                try:
                    relevant_memories = (
                        await self.semantic_memory_manager.query_memories(
                    ...
                    self.logger.warning(
                        "Non-critical error querying semantic memories: %s", mem_err
                    )

            # Update HTA with completion and memory context
            hta_result = await self.hta_service.update_task_completion(
                task_id=task_id,
                completion_data={**completion_data, "memory_context": memory_context},
            )
            ...
            if self.llm_client:
                try:
                    ...
                    self.logger.warning(
                        "Non-critical error generating insights: %s", insight_err
                    )
    ...
    async def _generate_completion_insights(
        self,
        task_id: str,
