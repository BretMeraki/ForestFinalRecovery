...
class TaskQueue:
    ...
    def __init__(self, max_workers: int = 10, result_ttl: int = 300):
        ...
        logger.info(
            "TaskQueue initialized with %d workers and %ds result TTL",
            max_workers,
            result_ttl,
        )
    ...
    async def _worker(self, worker_id: int):
        ...
        while self.running:
            try:
                ...
                self.task_metadata[task_id]["started_at"] = datetime.now(
                    timezone.utc
                ).isoformat()

                logger.debug(
                    "Worker %d processing task %s (priority: %d)",
                    worker_id,
                    task_id,
                    priority,
                )
                ...
                try:
                    ...
                    # Store the error
                    self.results[task_id] = {"status": "failed", "error": error_details}

                    # Update task metadata
                    self.task_metadata[task_id]["status"] = "failed"
                    self.task_metadata[task_id]["error"] = error_details
                    self.task_metadata[task_id]["failed_at"] = datetime.now(
                        timezone.utc
                    ).isoformat()
    ...
    async def _cleanup_results(self):
        ...
        while self.running:
            try:
                ...
                if expired_tasks:
                    logger.debug(
                        "Cleaned up %d expired task results", len(expired_tasks)
                    )
                ...
                logger.error("Error in result cleanup: %s", e)
    ...
    async def enqueue(
        ...
        **kwargs,
        ...
        logger.info("Task %s added to queue with priority %d", task_id, priority)
        return task_id
    ...
    async def get_result(
        ...
        while timeout is None or asyncio.get_event_loop().time() - start_time < timeout:
            ...
            if task_id in self.processing or any(
                task_id == item[1] for item in self.queue._queue
            ):
        ...
        # Timeout reached
        raise asyncio.TimeoutError("Timeout waiting for task %s", task_id)
