...
class HTANode:
    ...
    def update_completion(self) -> None:
        ...
        try:
            ...
            raise ValueError(f"Error calculating completion status: {exc}")
    ...
    def get_frontier_tasks(self) -> List["HTANode"]:
        ...
        try:
            ...
            raise ValueError(f"Error getting frontier tasks: {exc}")
...
class HTATree:
    ...
    def get_node(
        self, node_id: UUID, current_node: Optional[HTANode] = None
    ) -> Optional[HTANode]:
        if not isinstance(node_id, UUID):
            raise TypeError("node_id must be a UUID")
