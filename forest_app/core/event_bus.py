...
class EventBus:
    ...
    async def publish(self, event: Union[EventData, Dict[str, Any]]) -> str:
        ...
        logger.debug(
            "Published event %s of type %s to %s subscribers",
            event.event_id,
            event_type,
            len(all_subscribers),
        )
    ...
    async def _deliver_event(self, subscriber: Callable, event: EventData) -> None:
        ...
        try:
            ...
            logger.error(
                "Error delivering event %s to subscriber: %s", event.event_id, exc
            )
    ...
    def subscribe(
        ...
        logger.debug("Subscribed to event types: %s", event_types)
    ...
    def unsubscribe(self, callback: Callable) -> None:
        ...
        logger.debug("Unsubscribed from event types: %s", event_types)
    ...
    def decorator(func):
        if asyncio.iscoroutinefunction(func):
                ...
                try:
                    ...
                    if include_result:
                        ...
                        try:
                            ...
                            logger.debug("Could not include result in event: %s", exc)
                    ...
                    event_data["payload"]["error"] = str(exc)
                    event_data["payload"]["error_type"] = type(exc).__name__
            ...
            def wrapper(*args, **kwargs):
                ...
                logger.info(
                    "Would publish %s event for %s (sync functions can't publish events via event bus)",
                    event_type,
                    func.__name__,
                )
