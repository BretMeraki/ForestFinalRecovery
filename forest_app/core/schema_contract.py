...
class HTASchemaContract:
    ...
    validators: Dict[str, Callable[[Any], bool]] = {
        "node.status": lambda s: s
        in ["pending", "in_progress", "completed", "deferred", "cancelled"],
    ...
    def validate_relationship(
        ...
        if relationship_type in cls.relationship_rules:
            ...
            if "min_count" in rules and len(children_data) < rules["min_count"]:
                errors.append(
                    f"{relationship_type} requires at least {rules['min_count']} children"
                )

            if "max_count" in rules and len(children_data) > rules["max_count"]:
                errors.append(
                    f"{relationship_type} allows at most {rules['max_count']} children"
                )
            ...
            if "required_metadata" in rules:
                for child in children_data:
                    if "hta_metadata" not in child:
                        errors.append("Child node missing hta_metadata")
                        continue

                    for meta_field in rules["required_metadata"]:
                        if meta_field not in child.get("hta_metadata", {}):
                            errors.append(
                                f"Child node missing required metadata: {meta_field}"
                            )
    ...
    def optimize_tree_structure(
        cls, nodes_data: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        for node in nodes_data:
            ...
            if parent_id:
                parent_child_counts[parent_id] = (
                    parent_child_counts.get(parent_id, 0) + 1
                )
    ...
    def validate_task_dependencies(
        cls, node_dependencies: Dict[UUID, Set[UUID]]
    ) -> List[str]:
        """
