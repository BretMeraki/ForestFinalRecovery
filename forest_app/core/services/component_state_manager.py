...
class ComponentStateManager:
    ...
    def __init__(self, managed_engines: Dict[str, Any]):
        ...
        if not isinstance(managed_engines, dict):
            logger.error(
                "ComponentStateManager received non-dict for managed_engines. State management will likely fail."
            )
            # Optionally raise TypeError here
            # raise TypeError("managed_engines must be a dictionary.")
        logger.info(
            "ComponentStateManager initialized to manage components with keys: %s",
            list(managed_engines.keys()),
        )
    ...
    def load_states(self, snapshot: MemorySnapshot):
        ...
        """
        if not hasattr(snapshot, "component_state") or not isinstance(
            snapshot.component_state, dict
        ):
            logger.warning(
                "Snapshot component_state is missing or not a dict. Cannot load states."
            )
        ...
        for key, engine in self.managed_engines.items():
            # Get the state dict for this engine's key
            engine_state_dict = cs.get(key)
            ...
            if not isinstance(engine_state_dict, dict):
                logger.warning(
                    "Invalid state format found for component key '%s'. Expected dict, got %s. Skipping load.",
                    key,
                    type(engine_state_dict).__name__,
                )
            ...
            if (
                engine
                and hasattr(engine, "update_from_dict")
                and callable(engine.update_from_dict)
            ):
                # Check if it's a dummy service - avoid calling update_from_dict on dummies
                if type(engine).__name__ == "DummyService":
                    logger.debug(
                        "Skipping state load for component key '%s' because engine is a DummyService.",
                        key,
                    )
                    continue
                ...
                try:
                    logger.debug(
                        "Loading state for component key '%s' into %s...",
                        key,
                    ...
                    logger.exception(
                        "Failed to load state for component key '%s' into %s: %s",
                        key,
                        type(engine).__name__,
                        e,
                    )
                ...
                logger.debug(
                    "State found for key '%s', but engine %s lacks 'update_from_dict' method. Skipping load.",
                    key,
                    type(engine).__name__,
                )
    ...
    def save_states(self, snapshot: MemorySnapshot):
        ...
        if not hasattr(snapshot, "component_state") or not isinstance(
            ...
            logger.warning(
                "Snapshot component_state is missing or not a dict. Initializing before saving states."
            )
            ...
            if engine and hasattr(engine, "to_dict") and callable(engine.to_dict):
                ...
                if type(engine).__name__ == "DummyService":
                    ...
                    if key in cs:
                        del cs[key]
                        logger.debug(
                            "Removed potentially stale state for dummy component key '%s'.",
                            key,
                        )
                    continue
                ...
                try:
                    ...
                    if isinstance(engine_data, dict) and engine_data:
                        ...
                        del cs[key]
                        logger.debug(
                            "Removed empty/null state for component key '%s' from %s.",
                            key,
                            type(engine).__name__,
                        )
                    ...
                    logger.exception(
                        "Failed to get state dictionary using to_dict() for component key '%s' from %s: %s",
                        key,
                        type(engine).__name__,
                        e,
                    )
                ...
                logger.debug(
                    "Engine %s (key: '%s') lacks 'to_dict' method. Cannot save its state.",
                    type(engine).__name__,
                    key,
                )
        ...
        cs["last_activity_ts"] = datetime.now(timezone.utc).isoformat(
            timespec="seconds"
        )
