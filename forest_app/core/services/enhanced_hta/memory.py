...
from forest_app.models import HTANodeModel, MemorySnapshotModel
...
class HTAMemoryManager:
    ...
    async def store_memory(self, user_id: UUID, memory_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Stored memory snapshot {memory_snapshot.id} for user {user_id}"
            )
            return True
    ...
    async def get_latest_snapshot(self, user_id: UUID) -> Optional[MemorySnapshot]:
        ...
        try:
            async with self.session_manager.session() as session:
                ...
                logger.debug(
                    f"Retrieved memory snapshot {snapshot.id} for user {user_id}"
                )
                return snapshot
    ...
    async def update_memory_with_completion(
        self, user_id: UUID, node: HTANodeModel
    ) -> bool:
        ...
        try:
            ...
            completion_data = {
                ...
                "content": {
                    ...
                    "previous_state": (
                        current_snapshot.content if current_snapshot else {}
                    ),
                },
                ...
                "metadata": {
                    ...
                    "node_type": (
                        "major_phase"
                        if getattr(node, "is_major_phase", False)
                        else "task"
                    ),
                },
            }
            ...
            if success:
                logger.info(
                    f"Updated memory for user {user_id} with completion of node {node.id}"
                )
            else:
                logger.warning(
                    f"Failed to update memory for user {user_id} with node {node.id} completion"
                )
            ...
            logger.error(
                f"Error updating memory with completion for user {user_id}: {e}"
            )
            return False
