...
class EnhancedHTAService(HTAService):
    ...
    async def complete_node(self, node_id: UUID, user_id: UUID):
        ...
        """
        logger.info(
            "Processing node completion for node %s by user %s", node_id, user_id
        )
        ...
        if node.user_id != user_id:
            logger.error("User %s does not own node %s", user_id, node_id)
            raise ValueError("User %s does not own node %s" % (user_id, node_id))

        if node.status == "completed":
            logger.info("Node %s already completed", node_id)
            return {
        ...
        if not tree:
            logger.error("Tree %s not found", node.tree_id)
        ...
        if not success:
            logger.error("Failed to update node %s status", node_id)
