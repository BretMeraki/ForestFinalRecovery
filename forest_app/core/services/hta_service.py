...
class HTAService(HTAServiceProtocol):
    """Service for managing HTA (Hierarchical Task Analysis) with semantic memory integration.

    This service implements the HTAServiceProtocol, providing a standardized interface
    for managing task hierarchies, memory integration, and HTA evolution.
    """
    ...
    def __init__(self, llm_client, semantic_memory_manager):
        ...
        if not isinstance(
            self.semantic_memory_manager, SemanticMemoryManagerBase
        ):  # Fixed type check
            logger.critical(
                "HTAService initialized with invalid SemanticMemoryManager!"
            )

        logger.info("HTAService initialized with dependencies.")

    async def initialize_task_hierarchy(
        ...
        logger.info("Initialized HTA hierarchy for task %s", task_id)
    ...
    async def update_task_state(self, task_id: str, state_data: Dict[str, Any]) -> None:
        ...
        if task_id not in self.task_hierarchies:
            logger.warning("Task %s not found in hierarchies", task_id)
            return

        self.task_hierarchies[task_id].update(state_data)
        logger.debug("Updated state for task %s", task_id)
    ...
    async def _get_active_seed(self, snapshot: MemorySnapshot) -> Optional[Seed]:
        ...
        active_seed_id = snapshot.component_state.get("seed_manager", {}).get(
            "active_seed_id"
        )
        if active_seed_id and hasattr(self.semantic_memory_manager, "get_seed_by_id"):
            try:
                ...
                logger.warning(
                    "Could not get seed by ID %s from snapshot: %s", active_seed_id, e
                )
    ...
    async def load_tree(self, snapshot: MemorySnapshot) -> Optional[HTATree]:
        ...
        if (
            active_seed
            and hasattr(active_seed, "hta_tree")
            and isinstance(active_seed.hta_tree, dict)
            and active_seed.hta_tree.get("root")
        ):
            logger.debug(
                "Loading HTA tree from active seed ID: %s",
                getattr(active_seed, "seed_id", "N/A"),
            )
            current_hta_dict = active_seed.hta_tree
        # Priority 2: Load from snapshot core_state
        elif isinstance(snapshot.core_state, dict) and snapshot.core_state.get(
            "hta_tree"
        ):
            logger.warning(
                "Loading HTA tree from snapshot core_state (fallback)."
            )  # Log as warning - Seed should ideally be source of truth
            current_hta_dict = snapshot.core_state.get("hta_tree")
        else:
            logger.warning(
                "Could not find HTA tree dictionary in active seed or snapshot core_state."
            )
            return None
        ...
        if current_hta_dict and isinstance(current_hta_dict, dict):
            try:
                ...
                if tree and tree.root:
                    logger.info(
                        "Successfully loaded HTA tree with root: %s - '%s'",
                        tree.root.id,
                        tree.root.title,
                    )
                    return tree
                    ...
                    logger.error(
                        "Failed to parse valid HTATree object from loaded dictionary (root missing?)."
                    )
                    return None
            except ValueError as ve:
                logger.error("ValueError parsing HTA tree dictionary: %s", ve)
                return None
            except Exception as e:
                logger.exception("Unexpected error parsing HTA tree dictionary: %s", e)
                return None
        else:
    ...
    async def save_tree(self, snapshot: MemorySnapshot, tree: HTATree) -> bool:
        ...
        try:
            ...
            logger.exception("Failed to serialize HTATree object to dictionary: %s", e)
            return False

        if not final_hta_dict_to_save or not final_hta_dict_to_save.get("root"):
            logger.error(
                "Failed to serialize HTA tree or root node is missing in dict."
            )
            return False

        # 1. Update Snapshot Core State (Primary target)
        try:
            if not hasattr(snapshot, "core_state") or not isinstance(
                snapshot.core_state, dict
            ):
                snapshot.core_state = {}
            ...
            logger.exception("Failed to update HTA tree in snapshot core_state: %s", e)
        ...
        if active_seed and hasattr(self.semantic_memory_manager, "update_seed"):
            try:
                ...
                success = await self.semantic_memory_manager.update_seed(
                    active_seed.seed_id, hta_tree=final_hta_dict_to_save
                )
                if success:
                    logger.info(
                ...
                logger.exception(
                    "Failed to update seed %s with final HTA: %s",
                    active_seed.seed_id,
                    seed_update_err,
                )
            ...
            logger.error(
                "Cannot save HTA to seed: Injected SeedManager lacks update_seed method."
            )
        ...
        if snapshot_save_ok and active_seed and hasattr(active_seed, "hta_tree"):
            try:
                ...
                logger.exception(
                    "Error checking for meaningful moments: %s", moment_err
                )
    ...
    def update_node_status(self, tree: HTATree, node_id: str, new_status: str) -> bool:
        ...
        if node:
            ...
            logger.info(
                "Updating status for node '%s' (%s) to '%s'.",
                node.title,
                node_id,
                new_status,
            )
            ...
            if is_completion and is_meaningful_node:
                self._record_meaningful_transition(
                    node, tree
                )  # This doesn't modify the tree

            return True
        else:
            logger.warning(
                "Cannot update status: Node with id '%s' not found in tree.", node_id
            )
            return False  # Failure handled gracefully to protect the user experience

    async def evolve_tree(
        self, tree: HTATree, reflections: List[str], user_mood: Optional[str] = None
    ) -> Optional[HTATree]:
        """
        ...
        if not tree or not tree.root:
            ...
            return None
        if (
            not isinstance(self.llm_client, LLMClient)
            or type(self.llm_client).__name__ == "DummyService"
        ):
        ...
        if reflections and hasattr(self.llm_client, "distill_reflections"):
            logger.info(
                "Distilling %s reflections for evolution goal...", len(reflections)
            )
            try:
                distilled_response: Optional[DistilledReflectionResponse] = (
                    await self.llm_client.distill_reflections(reflections=reflections)
                )
                if distilled_response and distilled_response.distilled_text:
                    evolution_goal = distilled_response.distilled_text
                    logger.info(
                        "Using distilled reflection as evolution goal: '%s...'",
                        evolution_goal[:100],
                    )
                else:
                    logger.warning(
                        "Reflection distillation failed or returned empty text. Using default goal."
                    )
            except Exception as distill_err:
                logger.exception(
                    "Error during reflection distillation: %s. Using default goal.",
                    distill_err,
                )
        ...
        try:
            ...
            logger.debug(
                "Calling request_hta_evolution. Goal: '%s...'", evolution_goal[:100]
            )

            evolved_hta_response: Optional[HTAEvolveResponse] = (
                await self.llm_client.request_hta_evolution(
            ...
            if (
                not isinstance(evolved_hta_response, HTAEvolveResponse)
                or not evolved_hta_response.hta_root
            ):
                log_response = (
                    evolved_hta_response
                    if len(str(evolved_hta_response)) < 500
                    else str(type(evolved_hta_response))
                )
                logger.error(
                    "Failed to get valid evolved HTA from LLM. Type: %s. Response: %s",
                    type(evolved_hta_response),
                    log_response,
                )
            ...
            if llm_root_id != original_root_id:
                logger.error(
                    "LLM HTA evolution root ID mismatch ('%s' vs '%s'). Discarding evolved tree.",
                    llm_root_id,
                    original_root_id,
                )
            ...
            evolved_hta_root_dict = evolved_hta_response.hta_root.model_dump(
                mode="json"
            )
            ...
            try:
                ...
                if new_tree and new_tree.root:
                    ...
                    logger.error(
                        "Failed to re-parse evolved HTA dictionary into valid HTATree object."
                    )
                    return None  # Parsing failed
            except ValueError as ve:
                logger.error("ValueError parsing evolved HTA dictionary: %s", ve)
                return None
            except Exception as parse_err:
                logger.exception(
                    "Unexpected error parsing evolved HTA dictionary: %s", parse_err
                )
                return None

        except (LLMError, LLMValidationError) as llm_evolve_err:
            logger.error(
                "LLM/Validation Error during HTA evolution request: %s", llm_evolve_err
            )
            return None  # Evolution failed
        except Exception as evolve_err:
            logger.exception(
                "Unexpected error during HTA evolution process: %s", evolve_err
            )
            return None  # Evolution failed
    ...
    async def update_task_completion(
        ...
        try:
            ...
            update_result = await self._update_task_status(
                task_id=task_id, status="completed", context=memory_context
            )
            ...
            if memory_context and len(memory_context) > 100:
                ...
                importance = 0.8
            if update_result.get("progress_metrics", {}).get(
            ...
            content = "Completed '%s'" % update_result.get("title", "a task")
            if user_mood:
                content += " feeling %s" % user_mood
            ...
            self.logger.error("Error updating task completion: %s", e)
            raise
    ...
    async def get_task_history(self, task_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            memories = await self.semantic_memory_manager.query_memories(
                query="Get history for task %s" % task_id,
                k=10,
                event_types=["hta_update", "task_completion"],
            )
            return memories
        except Exception as e:
            self.logger.error("Error getting task history: %s", e)
            raise

    async def _update_task_status(
        self, task_id: str, status: str, context: str = ""
    ) -> Dict[str, Any]:
        """
        ...
        if task_id not in self.task_hierarchies:
            self.logger.warning(
                "Cannot update task %s: Not found in hierarchies", task_id
            )
            return {
                "success": False,
        ...
        self.logger.info(
            "Updated task '%s' (%s) status from '%s' to '%s'",
            title,
            task_id,
            old_status,
            status,
    ...
    async def _track_emotional_journey(
        self, task_id: str, user_mood: Optional[str], timestamp: str
    ) -> None:
        """
        ...
        if task_id not in self.task_hierarchies:
            self.logger.warning(
                "Cannot track emotional journey: Task %s not found", task_id
            )
            return
        ...
        task_data["emotional_journey"].append(
            {"mood": user_mood, "timestamp": timestamp}
        )

        # Update the hierarchy
        self.task_hierarchies[task_id] = task_data
        self.logger.debug(
            "Tracked emotional journey for task %s: %s", task_id, user_mood
        )
        ...
        if "children" in task_data and task_data["children"]:
            ...
            if metrics["total_steps"] > 0:
                metrics["completion_percentage"] = round(
                    (metrics["completed_steps"] / metrics["total_steps"]) * 100
                )
                ...
                if len(reflections) >= 2:
                    try:
                        ...
                        if first_timestamp and last_timestamp:
                            ...
                            duration_days = (last_dt - first_dt).total_seconds() / (
                                24 * 3600
                            )
                            duration_weeks = max(
                                duration_days / 7, 1
                            )  # At least 1 week to avoid division by zero

                            # Calculate velocity
                            metrics["recent_velocity"] = round(
                                len(reflections) / duration_weeks, 1
                            )
                    except (ValueError, IndexError) as e:
                        self.logger.warning("Error calculating velocity: %s", e)

        return metrics

    def _generate_supportive_message(
        self,
        task_title: str,
        old_status: str,
        new_status: str,
        ...
        if not is_completion:
            return (
                "Status updated to %s." % new_status
            )  # Simple message for non-completion updates

        # Create a pool of base messages
        base_messages = [
            "You've completed '%s'! Well done on taking this step forward.",
            "Wonderful progress! '%s' is now complete.",
            "'%s' is finished! Each step brings you closer to your goal.",
            "You've finished '%s'. This moment is worth celebrating.",
            "'%s' complete! Your journey continues to unfold beautifully.",
        ]
        ...
        if progress_metrics.get("milestone_reached"):
            ...
            if milestone_percentage == 100:
                enhancements.append(
                    "You've completed all the steps in this branch! What an accomplishment."
                )
            elif milestone_percentage == 75:
                enhancements.append(
                    "You're now 75% of the way through this branch. The end is in sight!"
                )
            elif milestone_percentage == 50:
                enhancements.append(
                    "You've reached the halfway point of this branch. Momentum is building!"
                )
            elif milestone_percentage == 25:
                enhancements.append(
                    "You've completed 25% of this branch. A strong start!"
                )
        ...
        if streak >= 3:
            enhancements.append(
                "You're on a %s-day streak! Consistency is transformative." % streak
            )
        ...
        if velocity >= 3:
            enhancements.append(
                "You're moving at an impressive pace of %s completions per week."
                % velocity
            )
        ...
        if enhancements:
            ...
            message = message % " ".join(selected_enhancements)
    ...
    def _is_meaningful_node(self, node: HTANode) -> bool:
        ...
        if node.title and any(
            keyword in node.title.lower() for keyword in meaningful_keywords
        ):
            return True
    ...
    def _record_meaningful_transition(self, node: HTANode, tree: HTATree) -> None:
        ...
        self.logger.debug("Recorded meaningful transition for node: %s", node.title)

    async def _check_for_meaningful_moments(
        ...
        for transition in tree._meaningful_transitions:
            ...
            if is_major_phase:
                message = "Major milestone achieved: %s" % node_title
                ...
                try:
                    ...
                    self.logger.info("Recorded major phase completion: %s", node_title)
                except Exception as e:
                    self.logger.error("Failed to record major phase completion: %s", e)
            ...
            # Other meaningful transitions (could expand with more specialized handling)
                ...
                try:
                    await self.semantic_memory_manager.store_memory(
                        event_type="progress",
                        content="Completed meaningful step: %s" % node_title,
                        metadata={
                            "transition": transition,
                            "timestamp": datetime.now(timezone.utc).isoformat(),
                        },
                    ...
                    self.logger.error(
                        "Failed to record meaningful step completion: %s", e
                    )
        ...
        try:
            ...
            for entry in history:
                if (
                    ...
                    and entry["metadata"].get("user_mood")
                ):
            ...
            history_text = "\n".join(
                [
                    ...
                    % (
                        ...
                        (
                            " (Mood: "
                            + m.get("metadata", {}).get("user_mood", "")
                            + ")"
                            if m.get("metadata", {}).get("user_mood")
                            else ""
                        ),
                    )
                    for m in history
            ...
            for line in lines:
                ...
                if "pattern" in line_lower:
                    ...
                    for marker in ["narrative", "journey", "story", "progress"]
                ):
            ...
            self.logger.error("Error analyzing task patterns: %s", e)
            raise
