...
class PhaseNotificationService:
    ...
    def find_next_phase(
        ...
        if not current_phase:
            logger.warning(
                "Cannot find next phase: current phase %s not found in manifest",
                current_phase_id,
            )
            return None
    ...
    def generate_phase_completion_event(
        ...
        if not completed_phase:
            error_msg = "Phase with ID %s not found in manifest" % completed_phase_id
        ...
        if not completed_phase.hta_metadata.get("is_major_phase", False):
            logger.warning(
                "Step '%s' (ID: %s) is not marked as a major phase. "
                "This may indicate inconsistent phase flagging in the manifest."
                % (completed_phase.title, completed_phase_id)
            )
        ...
        custom_message = (
            "Congratulations on completing the '%s' phase!" % completed_phase.title
        )
        if next_phase:
            custom_message += " Next up: %s" % next_phase.title
        else:
