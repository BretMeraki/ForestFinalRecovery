...
try:
    ...
    log_import_error(e, "dependencies.py")
    # Additional critical log
    logging.critical("Application dependencies failed to initialize correctly")
    imports_ok = False
...
def get_orchestrator(request: Request) -> ForestOrchestrator:
    ...
    try:
        ...
        if not isinstance(orchestrator, ForestOrchestrator):
            raise TypeError(
                "Container did not return a valid ForestOrchestrator instance."
            )
        return orchestrator
    except Exception as e:  # Catch potential errors during provider resolution
        logger.exception(
            "CRITICAL: Failed to get orchestrator from DI container: %s", e
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
def get_trigger_handler(request: Request) -> TriggerPhraseHandler:
    ...
    try:
        ...
        trigger_handler = (
            container.trigger_handler()
        )  # Assumes provider named 'trigger_handler' exists
        if not isinstance(trigger_handler, TriggerPhraseHandler):
            raise TypeError(
                "Container did not return a valid TriggerPhraseHandler instance."
            )
        return trigger_handler
    except Exception as e:  # Catch potential errors during provider resolution
        logger.exception(
            "CRITICAL: Failed to get trigger_handler from DI container: %s", e
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_request_context(
    request: Request, x_trace_id: Optional[str] = Header(None)
) -> RequestContext:
    ...
    return RequestContext(
        user_id=user_id, trace_id=trace_id, feature_flags=feature_flags
    )
...
def get_task_logger(db: Session = Depends(get_db)) -> TaskFootprintLogger:
    ...
    try:
        ...
        logger.error("Failed to create TaskFootprintLogger: %s", ve)
        raise HTTPException(
            status_code=500,
            detail="Internal server error: Could not initialize task logger.",
        )
...
def get_reflection_logger(db: Session = Depends(get_db)) -> ReflectionLogLogger:
    ...
    try:
        ...
        logger.error("Failed to create ReflectionLogLogger: %s", ve)
        raise HTTPException(
            status_code=500,
            detail="Internal server error: Could not initialize reflection logger.",
        )
