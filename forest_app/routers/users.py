...
async def read_users_me(
    ...
    logger.info(
        "Fetching details for current user ID: %d (%s)", user_id, current_user.email
    )
    ...
    try:
        ...
        if stored_model and stored_model.snapshot_data:
            try:
                ...
                logger.error(
                    "Error loading/parsing snapshot for user %d status: %s.",
                    user_id,
                    snap_load_err,
                    exc_info=True,
                )
            ...
            logger.warning(
                "Latest snapshot (ID: %d) for user %d has no data.",
                stored_model.id,
                user_id,
            )
        else:
            logger.info("No snapshot found for user %d.", user_id)
    except SQLAlchemyError as db_err:
        logger.error(
            "DB error fetching snapshot user %d status: %s",
            user_id,
            db_err,
            exc_info=True,
        )
    ...
    try:
        ...
        logger.error(
            "Validation failed creating final UserRead response user %d: %s",
            user_id,
            val_err,
        )
