...
class AppSettings(BaseSettings):
    ...
    FEATURE_ENABLE_XP_MASTERY: bool = (
        True  # Deferred Integration (Set default based on MVP decision)
    )
    FEATURE_ENABLE_MEMORY_SYSTEM: bool = (
        True  # Deferred Integration (Set default based on MVP decision)
    )
    ...
    FEATURE_ENABLE_HTA_VISUALIZATION: bool = (
        True  # <<< ADDED - Defaulting to True (Visible)
    )
    ...
    # Skipped Code Check (Use Levels)
    FEATURE_ENABLE_LOGGING_TRACKING: bool = True
    FEATURE_ENABLE_SNAPSHOT_FLOW: bool = (
        True  # Skipped Code Check (Process Always Runs)
    )
    FEATURE_ENABLE_METRICS_SPECIFIC: bool = (
        True  # Skipped Code Check (Considered Core/Not Flagged)
    )
    FEATURE_ENABLE_HARMONIC_FRAMEWORK: bool = (
...
for key, value in settings.model_dump().items():
    if "KEY" in key or "STRING" in key:
        ...
        logger.debug(">>> DEBUG SETTINGS: %s: %s" % (key, value))
