...
def migrate_user_id_column(conn):
    ...
    try:
        ...
        conn.execute(
            text("ALTER TABLE memory_snapshots ADD COLUMN user_id_int INTEGER")
        )
        logger.info("Added user_id_int column")

        # Convert string values to integers
        conn.execute(
            text("UPDATE memory_snapshots SET user_id_int = CAST(user_id AS INTEGER)")
        )
        ...
        conn.execute(
            text("ALTER TABLE memory_snapshots RENAME COLUMN user_id_int TO user_id")
        )
        ...
        conn.execute(
            text(
                "CREATE INDEX ix_memory_snapshots_user_id ON memory_snapshots (user_id)"
            )
        )
