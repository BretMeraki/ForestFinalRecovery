...
from typing import Any, Dict, List, Optional, Union  # <-- Add Union here
...
def get_user_by_email(db: Session, email: str) -> Optional[UserModel]:
    ...
    if not isinstance(db, Session):
        logger.error(
            "get_user_by_email called with invalid db session type: %s", type(db)
        )
        return None
    try:
        ...
        if not hasattr(UserModel, "email"):
            logger.error(
                "UserModel (potentially dummy) does not have 'email' attribute for query."
            )
            return None
        return db.query(UserModel).filter(UserModel.email == email).first()
    except SQLAlchemyError as e:
        logger.error(
            "Database error retrieving user by email %s: %s", email, e, exc_info=True
        )
        # Propagate the error for the endpoint handler to manage the transaction
        raise
    except Exception as e:
        logger.error(
            "Unexpected error retrieving user by email %s: %s", email, e, exc_info=True
        )
        # Propagate the error
        raise
...
def create_user(db: Session, user_data: Dict[str, Any]) -> Optional[UserModel]:
    ...
    try:
        ...
        if not hasattr(UserModel, "email"):  # Check for an expected attribute
            logger.error(
                "UserModel (potentially dummy) cannot be instantiated for user creation."
            )
            return None
        db_user = UserModel(
        ...
        db.refresh(db_user)
        logger.info(
            "Successfully created user with email: %s (ID: %d)",
            db_user.email,
            db_user.id,
        )
        return db_user
        ...
        TypeError
    ) as e:  # Catch TypeError specifically, e.g., if UserModel() takes no args
        logger.error(
            "TypeError during UserModel instantiation (likely import issue): %s",
            e,
            exc_info=True,
        )
        db.rollback()
        ...
        if (
            ...
            logger.warning(
                "Attempted to create user with existing email (DB constraint): %s",
                user_data.get("email"),
            )
...
class MemorySnapshotRepository:
    ...
    def __init__(self, db: Session):
        ...
        if not hasattr(MemorySnapshotModel, "user_id"):  # Check an expected attribute
            raise ImportError(
                "MemorySnapshotModel appears to be incompletely imported."
            )
        self.db = db
    ...
    def create_snapshot(
        ...
        **Does NOT commit the transaction.**
        ...
        try:
            ...
            logger.error(
                "TypeError during MemorySnapshotModel instantiation (likely import issue): %s",
                e,
                exc_info=True,
            )
    ...
    def get_latest_snapshot(self, user_id: UUID) -> Optional[MemorySnapshotModel]:
        ...
        try:
            ...
            snapshot = (
                ...
                .order_by(
                    getattr(MemorySnapshotModel, order_by_field).desc()
                )  # Order by available timestamp
                .first()
            )
    ...
    def update_snapshot(
        self,
        snapshot_model: MemorySnapshotModel,
        new_data: dict,
        ...
        **Does NOT commit the transaction.** Uses flag_modified for JSONB changes.
        ...
        if (
            not snapshot_model
            or not isinstance(snapshot_model, MemorySnapshotModel)
            or not hasattr(snapshot_model, "id")
        ):
            logger.warning(
                "Attempted to update a non-existent or invalid snapshot model."
            )
            return None
        ...
        try:
            ...
            logger.error(
                "AttributeError updating snapshot id %s (likely invalid model due to import or data issue): %s",
                getattr(snapshot_model, "id", "N/A"),
                e,
                exc_info=True,
            )
            ...
            logger.error(
                "Database error preparing update for snapshot id %s: %s",
                snapshot_model.id,
                e,
                exc_info=True,
            )
            ...
            logger.error(
                "Unexpected error preparing update for snapshot id %s: %s",
                snapshot_model.id,
                e,
                exc_info=True,
            )
    ...
    def get_snapshot_by_id(
        ...
        try:
            if not all(
                hasattr(MemorySnapshotModel, attr) for attr in ["id", "user_id"]
            ):
                logger.error(
                    "MemorySnapshotModel missing required attributes for query (id, user_id)."
                )
                raise AttributeError(
                    "MemorySnapshotModel missing required query attributes."
                )
    ...
    def delete_snapshot_by_id(self, snapshot_id: int, user_id: UUID) -> bool:
        ...
        try:
            ...
            if (
                snapshot_to_delete
            ):  # Check if a valid model was returned and belongs to user
            ...
            logger.error(
                "Database error deleting snapshot id %s: %s",
                snapshot_id,
                e,
                exc_info=True,
            )
            ...
            logger.error(
                "Unexpected error deleting snapshot id %s: %s",
                snapshot_id,
                e,
                exc_info=True,
            )
...
def get_latest_snapshot_model(
    ...
    try:
        # Determine the field to order by (prefer updated_at if it exists)
        order_by_field = (
            "updated_at" if hasattr(MemorySnapshotModel, "updated_at") else "created_at"
        )
        if not all(
            hasattr(MemorySnapshotModel, attr) for attr in ["user_id", order_by_field]
        ):
            logger.error(
                "MemorySnapshotModel missing required attributes (user_id, %s) for query.",
                order_by_field,
...
class HTATreeRepository:
    ...
    def get_tree(
        self, tree_id: Union[str, UUID], user_id: Union[str, UUID]
    ) -> Optional[Any]:
    ...
    def find_by_metadata(
        self, user_id: Union[str, UUID], metadata_key: str, metadata_value: Any
    ) -> Optional[Any]:
    ...
    def update_metadata(
        self, tree_id: Union[str, UUID], manifest: Dict[str, Any]
    ) -> bool:
...
class TaskEventLogRepository:
    ...
    def create_log(self, log_data: Dict[str, Any]) -> Optional[TaskFootprintModel]:
        ...
        required = [
            ...
            "task_id",
            "event_type",
        ]  # Define required fields for clarity
        if any(field not in log_data for field in required):
            logger.warning(
                f"Missing required fields for TaskFootprintModel: needed {required}, got {list(log_data.keys())}"
            )
            return None
        ...
        try:
            ...
            logger.error(
                "TypeError during TaskFootprintModel instantiation (check log_data keys match model fields): %s | Data: %s",
                e,
                log_data,
                exc_info=True,
            )
        ...
        try:
            ...
            self.db.rollback()
            logger.error(
                "Unexpected error creating task footprint log: %s", e, exc_info=True
            )
    ...
    def get_logs_for_task(self, task_id: str) -> List[TaskFootprintModel]:
        ...
        try:
            ...
            if not all(
                hasattr(TaskFootprintModel, attr) for attr in ["task_id", "timestamp"]
            ):
                logger.error(
                    "TaskFootprintModel missing required attributes for query."
                )
                return []
            # Query using the correct model name
            return (
                self.db.query(TaskFootprintModel)
                .filter(TaskFootprintModel.task_id == task_id)
                .order_by(TaskFootprintModel.timestamp.asc())
                .all()
            )
        except SQLAlchemyError as e:
            logger.error(
                "DB error getting task footprint logs for task %s: %s",
                task_id,
                e,
                exc_info=True,
            )
            ...
            logger.error(
                "Unexpected error getting task footprint logs for task %s: %s",
                task_id,
                e,
                exc_info=True,
            )
...
class ReflectionEventLogRepository:
    ...
    def create_log(self, log_data: Dict[str, Any]) -> Optional[ReflectionLogModel]:
        ...
        if any(field not in log_data for field in required):
            logger.warning(
                f"Missing required fields for ReflectionLogModel: needed {required}, got {list(log_data.keys())}"
            )
            return None
        ...
        try:
            ...
            logger.error(
                "TypeError during ReflectionLogModel instantiation (check log_data keys match model fields): %s | Data: %s",
                e,
                log_data,
                exc_info=True,
            )
        ...
        try:
            ...
            self.db.rollback()
            logger.error(
                "Unexpected error creating reflection log: %s", e, exc_info=True
            )
            return None

    # Using ReflectionLogModel for querying
    def get_logs_for_reflection(
        self, reflection_id: Union[str, int]
    ) -> List[ReflectionLogModel]:
        """
        ...
        """
        logger.warning(
            "get_logs_for_reflection filtering by 'reflection_id'. Verify this field exists on ReflectionLogModel."
        )
        # Determine the type of the ID if necessary for filtering
        # Change this if ReflectionLogModel uses a different identifier field name
        query_attr = "reflection_id"

        try:
            # Check attributes required for the intended query
            if not all(
                hasattr(ReflectionLogModel, attr) for attr in [query_attr, "timestamp"]
            ):
                logger.error(
                    "ReflectionLogModel missing required attributes for query ('%s', 'timestamp').",
                    query_attr,
                )
                return []
            # Query using the correct model name and attribute
            return (
                self.db.query(ReflectionLogModel)
                .filter(getattr(ReflectionLogModel, query_attr) == reflection_id)
                .order_by(ReflectionLogModel.timestamp.asc())
                .all()
            )
        except SQLAlchemyError as e:
            logger.error(
                "DB error getting reflection logs for ID %s: %s",
                reflection_id,
                e,
                exc_info=True,
            )
            ...
            logger.error(
                "Unexpected error getting reflection logs for ID %s: %s",
                reflection_id,
                e,
                exc_info=True,
            )
