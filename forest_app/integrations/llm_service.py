...
class GoogleGeminiService(BaseLLMService):
    ...
    def __init__(
        ...
        if (
            not self.model_name
            and settings_import_ok
            and hasattr(settings, "GEMINI_MODEL_NAME")
        ):
        ...
        if (
            not self.advanced_model_name
            and settings_import_ok
            and hasattr(settings, "GEMINI_ADVANCED_MODEL_NAME")
        ):
    ...
    def _create_generation_config(
        self, temperature: float = 0.7, max_tokens: int = 1000, json_mode: bool = False
    ) -> GenerationConfig:
    ...
    async def _process_response_async(self, response: GenerateContentResponse) -> str:
        ...
        if hasattr(candidate, "finish_reason"):
            if candidate.finish_reason == protos.Candidate.FinishReason.SAFETY:
                ...
                logger.warning(
                    "Response may contain recitation/copying from training data."
                )
    ...
    async def generate_text(
        ...
        if self.context_trimmer:
            prompt = self.trim_prompt_if_needed(
        ...
        try:
            ...
            model_name = (
                self.advanced_model_name if use_advanced_model else self.model_name
            )
    ...
    async def generate_json(
        ...
        # Check if we need to trim the prompt
        if self.context_trimmer:
            augmented_prompt = self.trim_prompt_if_needed(
                augmented_prompt, max_tokens=8000
            )  # Adjust based on model limits
    ...
    async def generate_structured_output(
        ...
        # Check if we need to trim the prompt
        if self.context_trimmer:
            structured_prompt = self.trim_prompt_if_needed(
                structured_prompt, max_tokens=8000
            )  # Adjust based on model limits
        ...
        try:
            ...
            raise LLMRequestError(
                f"Error generating structured output from Gemini: {e}"
            )
        finally:
            # Restore original settings
            if timeout is not None:
