"""Helper functions for Forest application."""
...
async def save_snapshot_with_codename(
    ...
    try:
        ...
        logger.error(
            "Error calling snapshot.to_dict(): %s. Cannot save snapshot.",
            dict_err,
            exc_info=True,
        )
    ...
    try:
        ...
        if isinstance(core_state_check, dict):
            ...
            logger.debug(
                "SAVE_SNAPSHOT: core_state['hta_tree'] presence check BEFORE save: %s",
                "PRESENT" if hta_tree_check != "MISSING_HTA_TREE" else hta_tree_check,
            )
        else:
            logger.debug(
                "SAVE_SNAPSHOT: core_state presence check BEFORE save: %s",
                core_state_check,
            )
        # Log the feature flags that were (hopefully) recorded
        logger.debug(
            "SAVE_SNAPSHOT: Recorded feature_flags: %s",
            updated_data.get("feature_flags", "MISSING_OR_ERROR"),
        )
    except Exception as log_err:
        logger.error("SAVE_SNAPSHOT: Error logging snapshot data: %s", log_err)

    generated_codename: str = (
        # Fallback
        f"Snapshot_{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}"
    )
    ...
    try:
        ...
        llm_response: Optional[SnapshotCodenameResponse] = await llm_client.generate(
            prompt_parts=[codename_prompt], response_model=SnapshotCodenameResponse
        )

        if isinstance(llm_response, SnapshotCodenameResponse) and llm_response.codename:
            temp_codename = llm_response.codename.strip()[
                : constants.MAX_CODENAME_LENGTH
            ]
            if temp_codename:
                ...
                logger.warning(
                    "LLM returned an empty codename after stripping. Using fallback."
                )
        else:
            logger.warning(
                "LLM did not return a valid SnapshotCodenameResponse object or codename was empty. Using fallback."
            )
        ...
        logger.warning(
            "LLM call for codename failed: %s. Using fallback.", llm_e, exc_info=False
        )
    ...
    try:
        ...
        if isinstance(hta_tree_to_log, dict) and "root" in hta_tree_to_log:
            ...
            logger.debug(
                "[HELPER PRE-REPO] Serialized core_state hta_tree structure being sent to repo:\n%s",
                log_data_str,
            )
        else:
            logger.debug(
                "[HELPER PRE-REPO] Serialized core_state hta_tree is missing or invalid before repo call."
            )

    except Exception as log_ex:
        logger.error(
            "[HELPER PRE-REPO] Error logging HTA state before repo call: %s", log_ex
        )
    # <<< --- END ADDED LOGGING --- >>>
    ...
    try:
        if action == "create":
            ...
            # Pass the serialized data dict
            new_or_updated_model = repo.update_snapshot(
                stored_model, updated_data, generated_codename
            )
