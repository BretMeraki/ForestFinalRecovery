...
try:
    ...
    print(
        "ERROR: Snapshot module could not import feature flags. Cannot reliably check features.",
        file=sys.stderr,
    )
    ...
    def is_enabled(feature) -> bool:
        return False  # Assume all off if flags missing
...
class SnapshotFlowController:
    ...
    def __init__(
        ...
        logger.info(
            "SnapshotFlowController initialized with frequency %d, max_snapshots %d",
            frequency,
            max_snapshots,
        )
    ...
    async def register_user_submission(self, full_snapshot: Any) -> Dict[str, Any]:
        ...
        self.counter += 1
        logger.debug(
            "User submission registered, counter at %d/%d", self.counter, self.frequency
        )
        ...
        if memory_content and self.semantic_memory_manager:
            try:
                ...
                logger.debug(
                    "Stored %d new memories from snapshot", len(memory_content)
                )
        ...
        if self.counter >= self.frequency:
                ...
                try:
                    ...
                    relevant_memories = (
                        await self.semantic_memory_manager.query_memories(
                            query=self._get_context_query(full_snapshot), k=5
                        )
                    )
                    ...
                    snapshot["memory_context"]["memory_themes"] = self._extract_themes(
                        relevant_memories
                    )
                except Exception as e:
            ...
            logger.info(
                "New snapshot stored with timestamp %s", snapshot.get("timestamp")
            )
            return snapshot
        return None
    ...
    def _get_context_query(self, snapshot: Any) -> str:
        ...
        try:
            ...
            if hasattr(snapshot, "task_backlog") and snapshot.task_backlog:
                current_tasks = [
                    task.get("title")
                    for task in snapshot.task_backlog[:3]
    ...
    async def save_snapshot(self, snapshot: MemorySnapshot) -> bool:
        ...
        try:
            ...
            if self.semantic_memory_manager:
                # Get recent memories
                recent_memories = (
                    await self.semantic_memory_manager.get_recent_memories(limit=5)
                )
                ...
                snapshot.update_memory_context(
                    ...
                    query_info={
                        ...
                        "timestamp": datetime.now(timezone.utc).isoformat(),
                        "relevance_score": (
                            sum(m.get("relevance", 0) for m in relevant_memories)
    ...
    def _build_context_query(self, snapshot: MemorySnapshot) -> str:
        ...
        if hasattr(snapshot, "current_batch_reflections"):
            recent_reflections = " ".join(
                snapshot.current_batch_reflections[-3:]
            )  # Last 3 reflections
        ...
        if hasattr(snapshot, "task_backlog"):
            active_tasks = [
                t.get("title", "")
                for t in snapshot.task_backlog[:3]
        ...
        if hasattr(snapshot, "shadow_score") and hasattr(snapshot, "capacity"):
            query_parts.append(
                f"Emotional state - Shadow: {snapshot.shadow_score:.2f}, Capacity: {snapshot.capacity:.2f}"
            )
    ...
    def _is_significant_snapshot(self, snapshot: MemorySnapshot) -> bool:
        ...
        try:
            ...
            if (
                hasattr(snapshot, "current_batch_reflections")
                and snapshot.current_batch_reflections
            ):
                return True
