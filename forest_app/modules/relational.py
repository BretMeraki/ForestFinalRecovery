...
try:
    ...
    logging.getLogger("relational_init").critical(
        "Pydantic not installed. Relational module requires Pydantic."
    )
...
try:
    ...
    logging.getLogger("relational_init").warning(
...
if pydantic_import_ok:
    ...
    class RepairActionResponse(BaseModel):
        ...
        # Add validation if specific tones required
        tone: str = Field(default="Gentle")
        scale: str = Field(
            default="Medium"
        )  # Add validation if specific scales required
    ...
    class RepairActionResponse:
        pass

    class ProfileUpdateResponse:
        pass

    class DeepeningSuggestionResponse:
        pass
...
class Profile:
    ...
    def update_emotional_tags(self, new_tags: dict):
        ...
        if not is_enabled(Feature.RELATIONAL):
            logger.debug(
                "Skipping Profile.update_emotional_tags for '%s': RELATIONAL feature disabled.",
                self.name,
            )
            return
        # --- End Check ---
        ...
        for tag, value in new_tags.items():
            try:
                ...
                delta = float(
                    value
                    # Assuming value itself is the delta or target? Let's assume delta.
                )
        ...
        logger.debug(
            "Profile '%s' emotional_tags updated to %s", self.name, self.emotional_tags
        )
    ...
    def update_connection_score(self, delta: float):
        ...
        if not is_enabled(Feature.RELATIONAL):
            logger.debug(
                "Skipping Profile.update_connection_score for '%s': RELATIONAL feature disabled.",
                self.name,
            )
            return
        # --- End Check ---
        try:
            ...
            self.connection_score = max(
                0.0, min(10.0, self.connection_score + delta_float)
            )
            if self.connection_score != old:  # Log only if changed
                logger.debug(
                    "Profile '%s' connection_score: %.2f → %.2f",
                    self.name,
                    old,
                    self.connection_score,
                )
        except (ValueError, TypeError):
    ...
    def update_love_language(self, new_love_language: str):
        ...
        if not is_enabled(Feature.RELATIONAL):
            logger.debug(
                "Skipping Profile.update_love_language for '%s': RELATIONAL feature disabled.",
                self.name,
            )
            return
        # --- End Check ---
    ...
    def from_dict(cls, data: dict) -> "Profile":
        ...
        if not isinstance(data, dict):
            return cls("Unknown_Error")  # Handle bad data
        ...
        if not isinstance(profile.love_language, str):
            profile.love_language = cls.DEFAULT_LOVE_LANGUAGE

        # Assumes string or None is ok
        profile.last_gifted = data.get("last_gifted")

        conn_score = data.get("connection_score", cls.DEFAULT_CONNECTION_SCORE)
        try:
            profile.connection_score = max(
                0.0, min(10.0, float(conn_score))
            )  # Validate and clamp
        except (ValueError, TypeError):
            profile.connection_score = cls.DEFAULT_CONNECTION_SCORE
...
class RelationalRepairEngine:
    ...
    def generate_repair_action(self, profile: Profile, context: str = "") -> dict:
        ...
        if not is_enabled(Feature.RELATIONAL):
            logger.debug(
                "Skipping generate_repair_action (static): RELATIONAL feature disabled."
            )
            return {}
        # --- End Check ---
        ...
        dominant_tag = max(
            profile.emotional_tags.items(),
            key=lambda kv: kv[1],
            default=("compassion", 0.0),
        )[0]
        score = profile.connection_score
        if score < 3.0:
            tone, action = "Cautious", f"Write letter expressing {dominant_tag}."
        elif score < 7.0:
            tone, action = "Gentle", f"Send note focusing on {dominant_tag}."
        else:
            tone, action = "Open", f"Reach out for conversation about {dominant_tag}."
        result = {
    ...
    async def generate_dynamic_repair_action(
        ...
        profile: Profile,
        snapshot: dict,
        context: str = "",  # LLMClient is optional
    ) -> dict:
        """Generates dynamic repair action via LLM. Returns static fallback if feature disabled or LLM fails."""
        # --- Feature Flag Check ---
        if not is_enabled(Feature.RELATIONAL):
            logger.debug(
                "Skipping generate_dynamic_repair_action: RELATIONAL feature disabled. Returning empty."
            )
        ...
        if (
            not llm_import_ok
            or not isinstance(llm_client, LLMClient)
            or not hasattr(llm_client, "generate")
        ):
            logger.warning(
                "LLMClient not available for dynamic repair action. Using static fallback."
            )
            return self.generate_repair_action(
                profile, context
            )  # Static check will return {} if flag off

        # Prune snapshot for prompt brevity
        pruned = {
            k: snapshot.get(k)
            for k in ["capacity", "shadow_score", "relationship_index"]
            if k in snapshot
        }  # Use relationship_index if available
        ...
        if pydantic_import_ok and issubclass(RepairActionResponse, BaseModel):
            try:
                response_model_schema = RepairActionResponse.model_json_schema(indent=0)
            except Exception:
                logger.error(
                    "Failed to generate Pydantic schema for RepairActionResponse"
                )
        ...
        try:
            llm_response: Optional[RepairActionResponse] = await llm_client.generate(
                prompt_parts=[prompt], response_model=RepairActionResponse
            )
            if isinstance(llm_response, RepairActionResponse):
                ...
                logger.warning(
                    "LLMClient did not return valid RepairActionResponse. Using static fallback."
                )
                result = self.generate_repair_action(profile, context)

        except (LLMError, LLMValidationError, ValidationError) as llm_e:
            logger.warning(
                "Dynamic repair action LLM/Validation error: %s. Using static fallback.",
                llm_e,
            )
            result = self.generate_repair_action(profile, context)
        except Exception as e:
            logger.exception(
                "Unexpected error during dynamic repair action: %s. Using static fallback.",
                e,
            )
            result = self.generate_repair_action(profile, context)
...
class RelationalManager:
    ...
    def __init__(self, llm_client: LLMClient):
        ...
        if not isinstance(llm_client, LLMClient) and llm_import_ok:
            raise TypeError(
                "RelationalManager requires a valid LLMClient instance unless LLM imports failed."
            )
        ...
        if not llm_import_ok:
            logger.error(
                "LLM Integrations failed import. RelationalManager LLM features disabled."
            )
    ...
    def add_or_update_profile(self, profile_data: dict) -> Optional[Profile]:
        ...
        if name in self.profiles:
            ...
            if "last_gifted" in profile_data:
                profile.last_gifted = profile_data["last_gifted"]
        else:
            try:
                ...
                if profile.name == "Unknown" and name:
                    ...
                    logger.warning(
                        "Profile name mismatch in data ('%s') vs key ('%s'). Using key.",
                        profile.name,
                        name,
                    )
                    profile.name = name
                ...
                logger.error("Failed to create Profile from dict: %s", e)
                return None
    ...
    def analyze_reflection_for_interactions(self, reflection_text: str) -> dict:
        ...
        default_signals = {
            "support": 0.0,
            "conflict": 0.0,
            "feedback": "Relational analysis disabled.",
        }
        # --- Feature Flag Check ---
        if not is_enabled(Feature.RELATIONAL):
            logger.debug(
                "Skipping analyze_reflection_for_interactions: RELATIONAL feature disabled."
            )
            return default_signals
        # --- End Check ---

        default_signals["feedback"] = (
            "No significant relational signals detected."  # Update default msg
        )
        ...
        conflict_score = sum(
            -0.1 for w in conflict_kw if re.search(rf"\b{w}\b", text)
        )  # Negative score
    ...
    async def infer_profile_updates(
        self, profile_name: str, reflection_text: str
    ) -> dict:
        ...
        if not profile:
            logger.warning(
                "Cannot infer updates, profile '%s' not found.", profile_name
            )
            return {}
        ...
        if pydantic_import_ok and issubclass(ProfileUpdateResponse, BaseModel):
            try:
                response_model_schema = ProfileUpdateResponse.model_json_schema(
                    indent=0
                )
            except Exception:
                logger.error(
                    "Failed to generate Pydantic schema for ProfileUpdateResponse"
                )
        ...
        updates = {}
        try:
            llm_response: Optional[ProfileUpdateResponse] = (
                await self.llm_client.generate(
                    prompt_parts=[prompt], response_model=ProfileUpdateResponse
                )
            )
            if isinstance(llm_response, ProfileUpdateResponse):
                ...
                logger.info(
                    "Profile '%s' inferred updates applied: %s", profile_name, updates
                )
            else:
    ...
    async def generate_repair_for_profile(
        self, name: str, snapshot: dict, context: str = ""
    ) -> dict:
        ...
        if not is_enabled(Feature.RELATIONAL):
            logger.debug(
                "Skipping generate_repair_for_profile: RELATIONAL feature disabled."
            )
            return {}
        # --- End Check ---
        ...
        return await self._repair_engine.generate_dynamic_repair_action(
            self.llm_client, profile, snapshot, context
        )

    async def generate_deepening_suggestion(
        self, name: str, snapshot: Optional[dict] = None, context: str = ""
    ) -> dict:  # snapshot optional
        """Generates deepening suggestion via LLM. Returns fallback if feature disabled or fails."""
        # --- Feature Flag Check ---
        if not is_enabled(Feature.RELATIONAL):
            logger.debug(
                "Skipping generate_deepening_suggestion: RELATIONAL feature disabled."
            )
            return {
                "deepening_suggestion": "Relational features currently offline.",
                "tone": "neutral",
            }
        # --- End Check ---
        ...
        if (
            ...
            return {
                "deepening_suggestion": "Suggestion engine offline.",
                "tone": "neutral",
            }
        ...
        if pydantic_import_ok and issubclass(DeepeningSuggestionResponse, BaseModel):
            try:
                response_model_schema = DeepeningSuggestionResponse.model_json_schema(
                    indent=0
                )
            except Exception:
                logger.error(
                    "Failed to generate Pydantic schema for DeepeningSuggestionResponse"
                )
        ...
        try:
            llm_response: Optional[DeepeningSuggestionResponse] = (
                await self.llm_client.generate(
                    prompt_parts=[prompt], response_model=DeepeningSuggestionResponse
                )
            )
            if isinstance(llm_response, DeepeningSuggestionResponse):
                ...
                logger.info(
                    "Deepening suggestion for '%s': %s",
                    name,
                    suggestion.get("deepening_suggestion"),
                )
            else:
                logger.warning(
                    "LLMClient did not return valid DeepeningSuggestionResponse."
                )
                suggestion = fallback
    ...
    def to_dict(self) -> dict:
        ...
        if not is_enabled(Feature.RELATIONAL):
            logger.debug(
                "Skipping RelationalManager serialization: RELATIONAL feature disabled."
            )
            return {}
        # --- End Check ---
    ...
    def update_from_dict(self, data: dict):
        ...
        if not is_enabled(Feature.RELATIONAL):
            logger.debug(
                "Resetting state via update_from_dict: RELATIONAL feature disabled."
            )
            self._reset_state()
            return
        # --- End Check ---
        ...
        if not isinstance(data, dict):
            logger.warning(
                "Invalid data type for RelationalManager.update_from_dict: %s",
                type(data),
            )
            self._reset_state()
            return
        ...
        if not isinstance(profiles_data, dict):
            logger.warning(
                "Invalid 'profiles' format in data: Expected dict, got %s.",
                type(profiles_data),
            )
            self._reset_state()
            return
        ...
        for name, pd in profiles_data.items():
            try:
                ...
                if (
                    prof.name == "Unknown_Error"
                ):  # Check for error during Profile creation
                    raise ValueError(f"Profile data for key '{name}' was invalid.")
                if prof.name != name and prof.name != "Unknown":
                    logger.warning(
                        "Profile name mismatch loading relational profiles: key '%s' vs data name '%s'. Using key.",
                        name,
                        prof.name,
                    )
        ...
        logger.info(
            "RelationalManager state updated from dict. Loaded %d profiles, errors on %d.",
            loaded_count,
            error_count,
        )
