...
class ShadowEngine:
    ...
    def analyze_text(
        ...
        if context:
            try:  # Add try-except for context processing
                ...
                logger.warning(
                    "Invalid context value type during lexicon adjustment: %s", ctx_err
                )
        ...
        try:
            for pattern, weight in self.pattern_lexicon.items():
                ...
                if matches:
                    ...
                    tag_name = (
                        # Truncate long patterns for tag name
        ...
        if "sentiment" in context:
            try:
                ...
                if sentiment < 0:
                    ...
                    tag_scores["negative_sentiment_factor"] = (
                        tag_scores.get("negative_sentiment_factor", 0)
        ...
        normalized_by_length = (
            abs(total_score) / (num_words + 1) if num_words > 0 else 0.0
        )
        ...
        return {
            "shadow_score": round(normalized_shadow, 3),
            "shadow_tags": tag_scores,
        }  # Increased precision
    ...
    def to_dict(self) -> dict:
        ...
        if not is_enabled(Feature.SHADOW_ANALYSIS):
            logger.debug(
                "Skipping ShadowEngine serialization: SHADOW_ANALYSIS feature disabled."
            )
            return {}
        # --- End Check ---
        ...
        return {
            ...
            # Assuming keys (regex) are immutable strings
            "pattern_lexicon": self.pattern_lexicon.copy(),
    ...
    def update_from_dict(self, data: dict):
        ...
        if not is_enabled(Feature.SHADOW_ANALYSIS):
            logger.debug(
                "Resetting state via update_from_dict: SHADOW_ANALYSIS feature disabled."
            )
        ...
        if not isinstance(data, dict):
            logger.warning(
                "Invalid data type passed to ShadowEngine.update_from_dict: %s. Resetting state.",
                type(data),
            )
        ...
        if isinstance(loaded_lexicon, dict):
            ...
            logger.warning(
                "Invalid 'lexicon' type in data: %s. Lexicon not updated.",
                type(loaded_lexicon),
            )
        ...
        if isinstance(loaded_patterns, dict):
            ...
            logger.warning(
                "Invalid 'pattern_lexicon' type in data: %s. Patterns not updated.",
                type(loaded_patterns),
            )
        ...
        if isinstance(loaded_negations, list):
            ...
            logger.warning(
                "Invalid 'negations' type in data: %s. Negations not updated.",
                type(loaded_negations),
            )
