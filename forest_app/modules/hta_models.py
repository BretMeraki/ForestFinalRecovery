...
from __future__ import \
    annotations  # Required for recursive type hints in Pydantic v1/v2
...
class HTANodeModel(BaseModel):
    ...
    title: str = Field(
        ..., max_length=150, description="Concise title for the HTA step."
    )
    # --- MODIFIED LINE BELOW ---
    description: Optional[str] = Field(
        default=None, description="Detailed description of the HTA step."
    )
    # --- END MODIFICATION ---
    ...
    depends_on: List[str] = Field(
        default=[], description="List of IDs of nodes this node depends on."
    )
    ...
    linked_tasks: List[str] = Field(
        default=[], description="List of task IDs linked to this node."
    )
    # --- NEW: Milestone Flag ---
    ...
    status_suggestion: Optional[str] = Field(
        default=None,
        # Added 'blocked'
        pattern="^(pending|active|completed|pruned|blocked)$",
        description="LLM's suggested initial status or a status loaded from state.",
    )

    # --- Recursive Children ---
    children: List[HTANodeModel] = Field(
        default=[], description="List of child HTA nodes."
    )

    @field_validator("depends_on")
    def check_depends_on_not_self(
        cls, v, info: ValidationInfo
    ):  # Updated signature for Pydantic v2
