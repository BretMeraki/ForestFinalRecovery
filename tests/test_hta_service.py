...
class TestHTAService:
    ...
    async def test_hta_minimal(self):
        ...
        try:
            ...
            result = await service.generate_initial_hta_from_manifest(
                manifest=manifest, user_id=user_id, request_context={"source": "test"}
            )
    ...
    async def test_generate_initial_hta_from_manifest_success(
        ...
        if not hasattr(hta_service.event_bus, "publish") or not callable(
            getattr(hta_service.event_bus, "publish")
        ):
        ...
        assert (
            execution_time < 1000
        ), f"Execution took {execution_time}ms, exceeding the 1s target"
    ...
    async def test_generate_initial_hta_from_manifest_db_error(
        ...
        hta_service.tree_repository.save_tree_model.side_effect = SQLAlchemyError(
            "Test DB error"
        )
        # Act & Assert
    ...
    async def test_generate_initial_hta_with_empty_manifest(
        ...
        empty_manifest = RoadmapManifest(
            tree_id=uuid.uuid4(), user_goal="Empty goal", q_and_a_responses=[], steps=[]
        )
        ...
        # Act
        result = await hta_service.generate_initial_hta_from_manifest(
            manifest=empty_manifest, user_id=user_id, request_context=request_context
        )
