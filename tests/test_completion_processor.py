...
async def test_idempotent_completion_returns_reinforcement():
    ...
    processor.tree_repository.get_task_footprint = AsyncMock(
        return_value=dummy_footprint
    )
...
async def test_successful_completion_updates_memory_and_returns_reinforcement():
    ...
    dummy_tree = DummyTree(
        ...
        manifest={"tree_id": str(dummy_node.tree_id), "user_goal": "dummy goal"},
    )
    ...
    processor.tree_repository.increment_branch_completion_count = AsyncMock(
        return_value=(True, 1)
    )
    ...
    processor.memory_manager.update_memory_with_completion = AsyncMock(
        return_value=True
    )
...
async def test_transaction_rollback_on_failure(monkeypatch):
    ...
    processor.tree_repository.get_node_by_id = AsyncMock(return_value=dummy_node)
    processor.tree_repository.get_tree_by_id = AsyncMock(return_value=dummy_tree)
    processor.tree_repository.update_node_status = AsyncMock(
        return_value=False
    )  # Simulate failure
