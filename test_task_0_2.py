...
class Task02TestSuite(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        db_url = os.environ.get("DB_CONNECTION_STRING") or os.environ.get(
            "DATABASE_URL"
        )
        if not db_url:
    ...
    def test_users_table_uuid_primary_key(self):
        ...
        self.assertEqual(
            len(pk_columns["constrained_columns"]),
            1,
            "users table should have exactly one primary key column",
        )
        self.assertEqual(
            pk_columns["constrained_columns"][0],
            "id",
            "Primary key column should be named 'id'",
        )

        columns = {c["name"]: c for c in self.inspector.get_columns("users")}
        self.assertIn("id", columns, "users table should have an 'id' column")
        self.assertEqual(
            columns["id"]["type"].__visit_name__,
            "UUID",
    ...
    def test_hta_trees_table_uuid_and_foreign_keys(self):
        ...
        self.assertIn(
            "user_id", columns, "hta_trees table should have a 'user_id' column"
        )
        self.assertEqual(
            columns["user_id"]["type"].__visit_name__,
            "UUID",
        ...
        user_fk = next(
            (fk for fk in fk_constraints if fk["referred_table"] == "users"), None
        )
        self.assertIsNotNone(user_fk, "hta_trees should have a foreign key to users")
        self.assertEqual(
            user_fk["referred_columns"],
            ["id"],
            "hta_trees.user_id should reference users.id",
        )
    ...
    def test_manifest_jsonb_field(self):
        ...
        columns = {c["name"]: c for c in self.inspector.get_columns("hta_trees")}
        self.assertIn(
            "manifest", columns, "hta_trees table should have a 'manifest' column"
        )
        self.assertEqual(
            columns["manifest"]["type"].__visit_name__,
            "JSONB",
            "manifest column should be JSONB type",
        )
    ...
    def test_manifest_gin_index(self):
        ...
        gin_index = next(
            (idx for idx in indexes if idx["name"] == "idx_hta_trees_manifest_gin"),
            None,
        )
        self.assertIsNotNone(
            gin_index,
            "GIN index 'idx_hta_trees_manifest_gin' should exist on hta_trees.manifest",
        )
        self.assertEqual(
            gin_index["dialect_options"]["postgresql_using"],
            "gin",
            "Index should use GIN",
        )

        # Verify the index is on manifest field
        self.assertEqual(
            gin_index["column_names"],
            ["manifest"],
            "GIN index should be on manifest column",
        )
    ...
    def test_hta_nodes_uuid_fields(self):
        ...
        for field_name in uuid_field_names:
            self.assertIn(
                field_name,
                columns,
                f"hta_nodes table should have a '{field_name}' column",
            )
            if field_name in columns:
                self.assertEqual(
                    columns[field_name]["type"].__visit_name__,
                    "UUID",
    ...
    def test_insert_with_uuid(self):
        ...
        with self.engine.connect() as connection:
            ...
            result = connection.execute(
                text("SELECT id, email FROM users WHERE id = :id"), {"id": test_id}
            )
            user = result.fetchone()

            self.assertIsNotNone(user, "User should be inserted")
            self.assertEqual(
                str(user.id), str(test_id), "Retrieved UUID should match inserted UUID"
            )
            self.assertEqual(
                user.email, test_email, "Retrieved email should match inserted email"
            )

            # Clean up
            connection.execute(
                text("DELETE FROM users WHERE id = :id"), {"id": test_id}
            )
            connection.commit()
