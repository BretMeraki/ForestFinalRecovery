...
def main():
    ...
    logger.info("-----------------------------")
    logger.info("Import checks: %s", "✓" if not import_results["failure"] else "✗")
    logger.info(
        "Dependency checks: %s",
        "✓" if not (dep_results["missing"] or dep_results["version_mismatch"]) else "✗",
    )
    logger.info("Environment variables: %s", "✓" if not env_results["missing"] else "✗")
    logger.info("File structure: %s", "✓" if not file_results["missing"] else "✗")
    logger.info(
        "Database migrations: %s", "✓" if migration_results.get("success") else "✗"
    )
