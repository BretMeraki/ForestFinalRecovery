...
async def run_test():
    ...
    async def mock_save_tree(tree_model):
        print(
            f"MOCK: Saving tree model with ID: {tree_model.id}, user_id: {tree_model.user_id}"
        )
        return tree_model
    ...
    try:
        ...
        async def mock_publish(event_type, event_data):
            ...
            if event_type not in list(EventType):
                print(
                    f"WARNING: event_type {event_type} is not in EventType enum: {list(EventType)}"
                )

            # Validate event_data.event_type matches event_type
            if (
                hasattr(event_data, "event_type")
                and event_data.event_type != event_type
            ):
                print(
                    f"WARNING: event_data.event_type {event_data.event_type} doesn't match event_type {event_type}"
                )
    ...
    SessionManager.get_instance = classmethod(
